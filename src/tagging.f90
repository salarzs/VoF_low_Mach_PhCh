module mod_tagging
  ...
  use mod_param     , only: dl,dims,lheat,datadir_ta,tmpg0,cbcvof,bcvof
  ...
  contains
  !
  subroutine droplet_tagging(n,e,h,dli,dzc,dzf,vof,delta,kappa,u,v,w,phi, &
                             tmp,tmpge,tmple,sca,mflux,pth,istep,time)
    !
    implicit none
    !
    ...
    real(8)   , dimension(0:n(1)+1,0:n(2)+1,0:n(3)+1) :: vof_f,vofTag_r
    ...
    real(8) :: dtmpdxp,dtmpdxm,dtmpdyp,dtmpdym,dtmpdzp,dtmpdzm, &
               ...
               dvofdx,dvofdy,dvofdz,tmpg_i,tmpl_i,s_in,d,yg_i,mfxu_i
    ...
    nd     = 0
    !
    ! assign to vof_f the initial volume-of-fluid distribution
    !
    vof_f(:,:,:)  = vof(:,:,:)
    vofTag(:,:,:) = 0
    !
    ...
    !  note: --> we prefer to do for real values
    !        --> the critical aspect is the conversion at the end
    !            from real(8) to int(8)
    !
    do k=1,n(3)
      do j=1,n(2)
        do i=1,n(1)
          vofTag_r(i,j,k) = 1.d0*vofTag(i,j,k)
        enddo
      enddo
    enddo
    call boundp(cbcvof,n,bcvof,dl,dzc,dzf,vofTag_r)
    do k=0,n(3)+1
      do j=0,n(2)+1
        do i=0,n(1)+1
    ...
    allocate(xd(nd),yd(nd),zd(nd),ud(nd),vd(nd),wd(nd),vold(nd), & 
             ...
             mfxd(nd),mfxu(nd),rhgd(nd),dlgd(nd),kagd(nd),tmpi(nd), &
        ...
        do i=1,n(1)
          ...
          if(vofTag(i,j,k).gt.0) then
            ...
            if( phi(ip,j,k)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(ip,j,k))+mflux(ip,j,k)*abs(phi(i,j,k)))/(abs(phi(ip,j,k))+abs(phi(i,j,k)))
            ...
            if( phi(im,j,k)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(im,j,k))+mflux(im,j,k)*abs(phi(i,j,k)))/(abs(phi(im,j,k))+abs(phi(i,j,k)))
              s_in   = mass_fraction(pth,tmpl_i)
              smad(id)    = max(s_in,smad(id))
              smid(id)    = min(s_in,smid(id))
              smed(id)    = smed(id) + s_in
            ...
            if( phi(i,jp,k)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(i,jp,k))+mflux(i,jp,k)*abs(phi(i,j,k)))/(abs(phi(i,jp,k))+abs(phi(i,j,k)))
              s_in   = mass_fraction(pth,tmpl_i)
              smad(id)    = max(s_in,smad(id))
              smid(id)    = min(s_in,smid(id))
              smed(id)    = smed(id) + s_in
            ...
            if( phi(i,jm,k)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(i,jm,k))+mflux(i,jm,k)*abs(phi(i,j,k)))/(abs(phi(i,jm,k))+abs(phi(i,j,k)))
              s_in   = mass_fraction(pth,tmpl_i)
              smad(id)    = max(s_in,smad(id))
              smid(id)    = min(s_in,smid(id))
              smed(id)    = smed(id) + s_in
            ...
            if( phi(i,j,kp)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(i,j,kp))+mflux(i,j,kp)*abs(phi(i,j,k)))/(abs(phi(i,j,kp))+abs(phi(i,j,k)))
              s_in   = mass_fraction(pth,tmpl_i)
              smad(id)    = max(s_in,smad(id))
              smid(id)    = min(s_in,smid(id))
              smed(id)    = smed(id) + s_in
            ...
            if( phi(i,j,km)*phi(i,j,k).lt.0. ) then
              ...
              mfxu_i = (mflux(i,j,k)*abs(phi(i,j,km))+mflux(i,j,km)*abs(phi(i,j,k)))/(abs(phi(i,j,km))+abs(phi(i,j,k)))
              s_in   = mass_fraction(pth,tmpl_i)
              smad(id)    = max(s_in,smad(id))
              smid(id)    = min(s_in,smid(id))
              smed(id)    = smed(id) + s_in
    ...
    do id=1,nd
      ...
      qkfd(id) = qkfd(id)                   ! we multiply by dl(1)*dl(2)*dl(3) when we compute qkfd
      !
      !print*, counter_p1
      area(id) = area(id)
      qksd(id) = qksd(id)
      mfxd(id) = mfxd(id)
      mfxu(id) = mfxu(id)/(1.d0*counter_p1(id))
    ...
    call write_output_r8(istep,'mfxd',mfxd,nd,'sum') ! mflux (kg/s)
    call write_output_r8(istep,'mfxu',mfxu,nd,'sum') ! mflux (kg/(s*m2))
